...
import selectCarefully from '../controllers/selectCarefully'
import classification from '../controllers/classification'
...
export default class Router {
   ...
   */
  bindLink() {
    $('.container>nav').on('click', 'a', this.handleLink.bind(this))
  }
  /**
   * history 处理a链接
   * @param  e 当前对象Event
   ...
   */
  navActive(item) {
    // let navItem = $('.container>nav a').filter(`[href="#${item}"]`)
    // let index = navItem.parent().index();
    // $('.container>nav a').filter(`[href="#${item}"]`).css({
    //   backgroundPosition: `${-index * 52}px -58px`
    // })
